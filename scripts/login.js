document.addEventListener('DOMContentLoaded', () => {
    ...
    // Datele default pentru produse (vor fi adaugate la fiecare comanda noua)
    ...
    // Functie pentru a transforma datele primite de la webhook
    const transformData = (rawData) => {
        return Object.keys(rawData).map(commandId => ({
            id: commandId,
            name: `Comanda #${commandId.substring(0, 12)}`, // Scurtam numele pentru afisare
            date: new Date().toLocaleDateString('ro-RO'),
            status: 'În Desfășurare',
            products: getDefaultProducts() // Adaugam produsele default
        }));
    };
